...
public class ScalableCoordinateSystem {
	...
	public ArrayList<DrawPolygon> getShapes() {
		return shapes;
	}

	public void setShapes(ArrayList<DrawPolygon> shapes) {
		this.shapes = shapes;
	}
	...
	public Pane drawClippingWindow() {
		 graph = new Pane();
	...
	public void drawAllClippedLines() {
		for (BresenhamPath p : bresenham_paths) {
			ArrayList<Double> path_points = bresenhamAlgorithm(p.getEndPoints());

			double[][] clippedLines = LineClipping.ClipThoseLines((int)(.2 * WIDTH), (int)(.8 * HEIGHT), (int)(.8 * WIDTH), (int)(.2 * HEIGHT), new double[][] {{path_points.get(0), path_points.get(1), path_points.get(path_points.size() - 2), path_points.get(path_points.size() - 1)}});
			
			for (int i = 0; i < path_points.size()-1; i+= 2) {
				Rectangle pixel = new Rectangle(path_points.get(i), path_points.get(i + 1), p.getThickness() + 1, p.getThickness() + 1);
				pixel.setFill(Color.RED);
				graph.getChildren().add(pixel);
			}

			double[] endPoints = clippedLines[0];
			double gradient = (endPoints[2] - endPoints[3]) /  (endPoints[0] - endPoints[1]);
			double yIntercept = endPoints[2] - (gradient * endPoints[0]);
			
			System.out.println(yIntercept);
			System.out.println(gradient);
			
			for (double x = Math.min(endPoints[0], endPoints[1]); x < Math.max(endPoints[0], endPoints[1]); x++) {
				double y = (gradient * x) + yIntercept; 
				Rectangle pixel = new Rectangle(x, y, p.getThickness() + 1, p.getThickness() + 1);
	...
	public void remove_last_irregular_shape() {
		if (shapes.size() > 0 && shape_state.size() > 0) {
			if (!shapes.get(shapes.size() - 1).hasPointCircles()) remove_last_irregular_shape_without_point_circles();
			else{
	...
	public void remove_last_irregular_shape_without_point_circles() {
		if (shapes.size() > 0) {
			 shapes.remove(shapes.size() - 1);
		}
	}
...
private void createClippingWindowAxes() {
		
		Line verticalLeft = new Line(.2 * WIDTH, 0, .2 * WIDTH, HEIGHT);
		verticalLeft.setStroke(Color.BLACK);
		
		graph.getChildren().add(verticalLeft);
		
		Line verticalRight = new Line(.8 * WIDTH, 0, .8 * WIDTH, HEIGHT);
		verticalRight.setStroke(Color.BLACK);
		
		graph.getChildren().add(verticalRight);
		
		Line horizontalTop = new Line(0, .2 * HEIGHT, WIDTH, .2 * HEIGHT);
		horizontalTop.setStroke(Color.BLACK);
		
		graph.getChildren().add(horizontalTop);
		
		Line horizontalBottom = new Line(0, .8 * HEIGHT, WIDTH, .8 * HEIGHT);
		horizontalBottom.setStroke(Color.BLACK);
		
		graph.getChildren().add(horizontalBottom);
	...
	private void drawAllShapes() {
		for (DrawPolygon p : shapes) {
			double[] newPosition = getRealPosition(p.getPoints());
			Polygon polygon = new Polygon(newPosition);
			...
			if (!p.getScanlineFillPixels().isEmpty() && !p.getIsAnimationOn()) {
				p.reworkScanlinePixelsToFill(newPosition);
				
				for (ArrayList<Integer> scanlineY : p.getScanlineFillPixels()) {
